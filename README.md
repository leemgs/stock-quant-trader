...
### 💰 10배 성장을 위한 3대 원칙
1. **복리의 극대화 (Compounding)**: `CompoundManager`를 활성화하여 수익이 발생할 때마다 매수 강도를 높이세요.
2. **주도주 집중 (Selection)**: 거래량이 전일 대비 500% 이상 폭증하는 종목에만 집중하여 자금 회전율을 높이세요.
3. **손절의 기계화 (Protection)**: -1.5% 손절선을 엄격히 준수하여 원금 파손을 방지하세요.
